...
DirectX 11 그래픽스 추상화 레이어 래퍼 라이브러리.
...
- 윈도우 리사이즈 지원
- 풀스크린 전환은 미구현 (`SetFullscreenState` 경로 없음, Alt+Enter 비활성화)

### 2D 스프라이트 엔진
- **C_DX11_SPRITE_ENGINE** - 인스턴싱 기반 스프라이트 배칭 렌더러 (싱글톤)
- 배치당 최대 16,384개 스프라이트 처리 (상한 도달 시 플러시 후 같은 프레임에서 계속 처리)
- 텍스처 슬롯 4,096 상수는 정의되어 있으나 런타임 강제는 미구현
- 텍스처 변경 시 자동 플러시
- 동적 텍스처 업데이트 (애니메이션)
- 스크린샷 저장 (BMP)
...
핵심 DX11 래퍼 타입과 함수는 `dx11` 네임스페이스에 정의된다. ImGui 통합 클래스 `C_IMGUI`는 전역 네임스페이스에 있다.
